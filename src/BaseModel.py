...
class BaseModel(object):
    def __init__(self,
        ...
        with tf.variable_scope(scope or "build_network"):
            with tf.variable_scope("decoder/output_projection"):
                self.output_layer = layers_core.Dense(self.hparams.target_vocab_size, use_bias=False,
        ...
        if self.mode == tf.contrib.learn.ModeKeys.TRAIN:
            ...
            self.infer_logits, _, self.final_context_state, self.sample_is = graph_res
    ...
    def build_graph(self, scope=None):
        ...
        with tf.variable_scope(scope or "dynamic_seq2seq", dtype=dtype):
            ...
            if self.mode != tf.contrib.learn.ModKeys.INFER:
    ...
    def _build_decoder(self, encoder_outputs, encoder_state):
        tgt_sos_id = tf.cast(self.tgt_vocab_table.lookup(tf.constant(self.hparams.sos)),
                             tf.int32)
        tgt_eos_id = tf.cast(self.tgt_vocab_table.lookup(tf.constant(self.hparams.eos)),
                             tf.int32)
        ...
        with tf.variable_scope("decoder") as decoder_scope:
            ...
            if self.mode != tf.contrib.learn.ModeKeys.INFER:
                target_input = self.target_vocab
                if self.time_major:
